...
def decrypt_packet(data: bytes) -> dict:
    try:
        ...
        raise ValueError(f"No se pudo descifrar el paquete: {e}")
...
def handle_incoming(conn, addr, save_dir):
    sender_ip = addr[0]
    try:
        # Leer longitud con timeout de lectura
        conn.settimeout(15)
        raw_len = b""
        while len(raw_len) < 4:
        ...
        # Límite de seguridad: máximo 100 MB por paquete
        MAX_SIZE = 100 * 1024 * 1024
        if data_len > MAX_SIZE:
            print(f"\n  {C.R}[ghost-send] Paquete demasiado grande: {data_len} bytes (máx 100MB){C.RESET}")
            conn.sendall(b"ACK:ERR")
            conn.close(); return
        ...
        if len(data) < data_len:
            print(f"\n  {C.R}[ghost-send] Paquete incompleto: {len(data)}/{data_len} bytes{C.RESET}")
            conn.sendall(b"ACK:ERR")
            conn.close(); return
        ...
        try:
            ...
            print(f"\n  {C.R}[ghost-send] Error de cifrado: {e}{C.RESET}")
            print(f"  {C.R}  Asegúrate de que emisor y receptor usan la misma versión de ghost-send.{C.RESET}")
            conn.sendall(b"ACK:ERR")
            conn.close(); return
